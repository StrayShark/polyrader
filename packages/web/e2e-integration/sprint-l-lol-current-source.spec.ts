...
test.skip(!enabled, 'Set POLYRADER_REAL_SOURCE_E2E=1 to run Sprint L LoL current-source tracking');
test.setTimeout(300_000);

test('Sprint L LoL audit records lifecycle without fabricating settlement', async ({
  request,
}, testInfo) => {
  const syncResponse = await request.post('/api/esports/sources/lol/sync');
  expect(syncResponse.ok()).toBe(true);

  const auditResponse = await request.post('/api/validation-lab/release-audits/lol', {
  ...
  await testInfo.attach('sprint-l-lol-audit', {
  ...
  const lifecycleResponse = await request.get('/api/validation-lab/lifecycle/lol');
  ...
  await testInfo.attach('sprint-l-lol-lifecycle', {
