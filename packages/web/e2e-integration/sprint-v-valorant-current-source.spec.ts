...
test.skip(
  !enabled,
  'Set POLYRADER_REAL_SOURCE_E2E=1 to run Sprint V Valorant current-source tracking',
);
test.setTimeout(300_000);

test('Sprint V Valorant audit records lifecycle without fabricating settlement', async ({
  request,
}, testInfo) => {
  const syncResponse = await request.post('/api/esports/sources/valorant/sync');
  expect(syncResponse.ok()).toBe(true);

  const auditResponse = await request.post('/api/validation-lab/release-audits/valorant', {
  ...
  await testInfo.attach('sprint-v-valorant-audit', {
  ...
  const lifecycleResponse = await request.get('/api/validation-lab/lifecycle/valorant');
  ...
  await testInfo.attach('sprint-v-valorant-lifecycle', {
