...
describe('MinimaxClient', () => {
        ...
        choices: [
          {
            message: {
              content:
                '{"winProbability":{"yes":0.6,"no":0.4},"confidence":70,"reasoning":"测试","keyFactors":["因素1"],"riskAssessment":"低风险"}',
              reasoning_details: [{ content: '思考1' }, { content: '思考2' }],
            },
          },
        ],
  ...
  it('API 错误时抛出异常', async () => {
    ...
    await expect(
      client.analyze({ system: '系统', context: '上下文', outputSchema: 'schema' }),
